...
echo Reporting PAPI_TOT_INS_MEAN
echo
...
do
	...
	sed -ie "s#^  //\(typedef SimpleLogger Logger;\)#  \1#" src/lib/io/logging.h
	make -j $PROCESSORS > /dev/null 2>/dev/null
	[ $? -ne 0 ] && echo make failed && exit
	printf "%-20s" $LOGGER
	HYRISE_DB_PATH=`pwd` LD_LIBRARY_PATH=`pwd`/build build/perf_regression --gtest_filter=*Insert* --gtest_catch_exceptions=0 2>/dev/null | grep PAPI_TOT_INS_MEAN | cut -d ":" -f 2 | tr -d " " | xargs printf "%25.0f"
	echo
